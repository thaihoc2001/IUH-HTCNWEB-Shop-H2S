...
$(document).ready(function (){
    $.getJSON("../../../js/data.json", function (data) {
        ...
        datamale = dataProducts.filter(function(item) {
            return item.gender == "male";
        })
        console.log(datamale)
        data1 = datamale.filter(function (item) {
            return item.classify == "adidas"
        })
        console.log(data1)
        data2 = datamale.filter(function (item) {
            return item.classify == "converse"
        })
        data3 = datamale.filter(function (item) {
            return item.classify == "mlb"
        })
        data4 = datamale.filter(function (item) {
            return item.classify == "vans"
        })
        data5 = datamale.filter(function (item) {
            return item.classify == "nike"
        })
        data6 = datamale.filter(function (item) {
            return item.price < 1000000;
        })
        data7 = datamale.filter(function (item) {
            return item.price > 1000000 && item.price < 3000000;
        })
        data8 = datamale.filter(function (item) {
            return item.price > 3000000 && item.price < 5000000;
        })
        data9 = datamale.filter(function (item) {
    ...
    function renderProdcut(){
        $("#check-adidas").click(function (){
            if(this.checked){
                ...
                DisplayList(datamale, list_element, rows, current_page);
                SetupPagination(datamale, pagination_element, rows);
            }
        })
